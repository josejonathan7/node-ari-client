    ...
    return new Promise((resolve, reject) => {
      ...
      fetch(ariUrl)
        ...
        .then((res) => {
            this._swagger = swagger({
              ...
              authorizations: {
                basicAuth: {
                  username: this._connection.user,
                  password: this._connection.pass
                }
              }
            })
        })
        .catch(err => {
          if (err && ['ETIMEDOUT', 'ENOTFOUND', 'ECONNREFUSED', 'EHOSTUNREACH'].indexOf(err["cause"].code) !== -1) {
      ...
      const attachResource = (resource) => {
        ...
        const attachOperation = (operation) => {
          this[resource][operation] = callSwagger;
           ...
           *  @returns {Q} promise - a promise that will resolve to a client
           */
          const callSwagger = (/* args..., callback */) => {
            ...
            return new Promise(function(innerResolve, innerReject) {
              ...
              this._swagger.apis[resource][operation].apply(null, args);
