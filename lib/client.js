            ...
            await swagger(request)
              .then(async client => {
                this._swagger = {
                 ...
                 for (const requestConfig of Array.from(map.values())) {
                  for (const key of _resources.knownTypes) {
                    const lowerCaseFirstLetter = key.charAt(0).toLowerCase() + key.slice(1)
                    const knownTypes = `${lowerCaseFirstLetter}s`;
                    ...
                    for (const operation of requestConfig.operations) {
                      ...
                      const queryParamsArray = params.filter(key => key.paramType === "query");
                      const bodyParamsArray = params.filter(key => key.paramType === "body");
                      const pathParamsArray = params.filter(key => key.paramType === "path");

                      const queryParams = queryParamsArray.length ? queryParamsArray.map(queryParam => {
                        //console.log(k.allowableValues);
                        return queryParam.name

                      }).join("=&") : undefined


                      const swaggerReq = swagger({
                        url: ariUrl,
                        requestInterceptor: (req) => {
                          let formatUrl = req.url.split("/api-docs")[0].concat(requestConfig.path)

                          for (const path of pathParamsArray) {
                            if (path.name === "configClass") {
                              formatUrl = formatUrl.replace(`{${path.name}}`, "res_pjsip");
                              continue;
                            }

                            if (path.name === "objectType") {
                              formatUrl = formatUrl.replace(`{${path.name}}`, knownTypes);
                              continue;
                            }

                            if (path.name === "id") {
                              formatUrl = formatUrl.replace(`{${path.name}}`, "teste");
                              continue;
                            }
                          }
                          ...
                          req.url = queryParams ? formatUrl.concat(`?${queryParams}`) : formatUrl
                        ...
                        .then(res => res)
                        .catch(err => {
                          //console.log("errona busca de recurso: ", err);

                          return {}
                        })

                      this[knownTypes][nickName] = swaggerReq
